    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
          if(values.newPassword ===values.currentPassword) {
            toast.error("New password can't be the same as current password", {duration:3000 , position: "bottom-left"  } );
            return;
          }
          ...
          .then((res) => {
            if (res.status === 204)
            toast.success("Password Changed successfully!", {duration:3000 , position: "bottom-left"  } );
          })
          .catch((err) => {
            console.log(err)
            if( err.response.status === 404 || err.response.status === 401){
              toast.error(err.response.data, {duration:3000 , position: "bottom-left"  });
            }
          });
        },
        })
